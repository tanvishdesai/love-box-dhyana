...
const createInfiniteItems = (originalItems: CarouselItem[]) => {
  const items: (Omit<CarouselItem, "id"> & { originalIndex: number; id: string })[] = [];
  for (let i = 0; i < 3; i++) {
...
export function RulerCarousel({
  ...
  return (
    <div className="w-full h-[600px] flex flex-col items-center justify-center overflow-hidden">
            ...
            {infiniteItems.map((item, index) => {
              ...
              return (
                <motion.button
                  ...
                  style={{
                    width: "400px",
                    color: isActive ? (item.color || "currentColor") : undefined
                  }}
                >
                  {item.title}
                </motion.button>
